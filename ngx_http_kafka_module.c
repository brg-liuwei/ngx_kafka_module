...
typedef struct {
    ...
    size_t           message_max_bytes; /* for rd_kafka_conf_t: default 1000000 */
...
void *ngx_http_kafka_create_main_conf(ngx_conf_t *cf)
{
    ...
    conf->message_max_bytes = NGX_CONF_UNSET_SIZE;
    ...
    ngx_http_kafka_main_conf_t *main_conf)
{
    ...
    if (main_conf->message_max_bytes == NGX_CONF_UNSET_SIZE)
        return; /* no option */

    ngx_snprintf(buf, sizeof(buf), "%lu%Z", main_conf->message_max_bytes);
